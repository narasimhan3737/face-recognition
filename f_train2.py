...
for r,d,Fs in os.walk(imdir): #browsing on image directories
    for fl in Fs: 
        if ((fl=="face.jpg") or (fl.endswith(".png"))): # if the found file is face.jpg or a .png file
            ...
            ey = eye.detectMultiScale(img_array,1.3,5)#detect eye and recieve co-ordinates
            sl = sml.detectMultiScale(img_array,3,15)#detect smile and recieve co-ordinates
            for (x,y,w,h) in f: 
...
with open("db/lables2.pickle",'wb') as f:
    pickle.dump(l_id,f)

recog.train(x_t, np.array(y_l))
recog.save("db/trainer2.yml")
